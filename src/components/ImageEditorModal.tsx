...
import { X, Upload, Link as LinkIcon, RotateCcw, Sparkles, Camera, Plus, Trash2, Images, Disc, CheckCircle2, Image as ImageIcon, ArrowUp, ArrowDown, Check } from 'lucide-react';
import { useBandImages, KungnoiImageMap, PRESET_LIBRARY } from '../context/ImageContext';
          ...
          <div className="md:col-span-8 space-y-4">
              ...
              <div className="bg-neutral-950 border border-neutral-800 rounded-xl p-3 space-y-2">
                ...
                {isAddingToSlideshow && (
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 pt-1">
                    <input
                      type="text"
                      placeholder="หัวข้อสไลด์ (เช่น Kungnoi Y. Sunset Live)"
                      value={slideTitle}
                    ...
                    <input
                      type="text"
                      placeholder="คำบรรยายสไลด์ (เช่น Dreamy Soul Pop • Mozart Music)"
                      value={slideSubtitle}
            ...
            {activeTab === 'slideshow_list' && (
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                  <label className="text-xs font-mono text-amber-400 block font-bold">
                    สไลด์ทั้งหมดในหน้าแรก ({slideshowList.length} รูป):
                  </label>
                ...
                <div className="space-y-2 max-h-80 overflow-y-auto pr-1">
                  {slideshowList.map((slide, index) => (
                    <div
                      key={slide.id || index}
                      className="bg-neutral-950 border border-neutral-800/90 hover:border-neutral-700 rounded-xl p-2.5 flex items-center justify-between gap-2 sm:gap-3 transition-colors"
                    >
                      ...
                      <div className="flex items-center gap-1.5 flex-shrink-0">
                        {slide.tag && (
                          <span className="hidden sm:inline-block text-[9px] font-mono bg-neutral-900 text-neutral-300 border border-neutral-800 px-1.5 py-0.5 rounded truncate max-w-[110px]">
                            {slide.tag}
                          </span>
                        )}
                        <button
